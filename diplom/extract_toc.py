...
def extract_page_number(lines):
    for i in range(len(lines) - 1, max(len(lines) - 6, -1), -1):
        m = re.search(
            r"ДП\.АС64\.\d{6}-\d{2}\s+\d{2}\s+\d{2}\s+(\d+)\s*$", lines[i]
        )
        if m:
...
def is_chapter_heading(line):
    ...
    if any(
        token in stripped
        for token in ("Изм.", "Лист", "ДП.", "Подп.", "Дата")
    ):
        return False
...
def is_title_continuation(line):
    stripped = line.strip()
    return (
        bool(re.match(r"^[а-я][а-яё\s\-—–,\d]+$", stripped))
        and len(stripped) < 60
    )
...
def main():
    # ==================== НАСТРОЙКИ ПУТЕЙ ====================
    # Укажите папку, где лежит ваш PDF (можно абсолютный или относительный путь)
    TARGET_DIR = "/mnt/UbuntuHost/vlad/IdeaProjects/DiplomPasterisator/diplom"

    # Имя входного файла
    INPUT_PDF = "main.pdf"

    # Путь к выходному файлу (сохранится в TARGET_DIR, если указано только имя)
    OUTPUT_FILE = "toc_output.tex"
    # =========================================================

    # Собираем полный путь к PDF
    ...
    # Формируем итоговый текст для записи
    output_lines = []
    ...
    # Определяем, куда сохранять выходной файл
